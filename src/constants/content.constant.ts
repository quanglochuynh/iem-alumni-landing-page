import { AppRoute } from '@/interfaces';

export const NAV_ITEMS = [
  { href: AppRoute.HOME, label: 'Home' },
  { href: AppRoute.WIEMA2025, label: 'WIEMA 2025' },
  { href: AppRoute.ARTICLES, label: 'Blogs' },
  { href: AppRoute.CAREERS, label: 'Careers' },
  { href: AppRoute.EVENTS, label: 'Events' },
];

export const HERO = {
  image: '/wiema-cover.webp',
  rsvpUrl: 'https://forms.gle/LqCuU8F2haZRd51y5',
};

export const FOOTER = {
  mapSrc:
    'https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d659.7029922633542!2d106.80159818456582!3d10.877464066879655!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3174d8a415a9d221%3A0x550c2b41569376f9!2sInternational%20University%20-%20VNU-HCM!5e0!3m2!1sen!2s!4v1762787522936!5m2!1sen!2s',
  copyright: 'Copyright © 2025 IEM Alumni Association. All rights reserved.',
  suggestEditsUrl: 'https://github.com/quanglochuynh/iem-alumni-landing-page',
};
